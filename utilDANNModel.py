...
class DANNModel(object):
    ...
    # -------------------------------------------------------------------------
    def compile(self):
        self.dann_model.compile(loss={'out_states': 'categorical_crossentropy','domain_output': 'categorical_crossentropy'},
    ...
    def __build_dann_model(self):
        ...
        branch_label = Lambda(lambda x: K.switch(K.learning_phase(),K.concatenate([x[:int(self.batch_size//2)],x[:int(self.batch_size//2)]], axis=0),x),output_shape=lambda x: x[0:])(branch_features)
